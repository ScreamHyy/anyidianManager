...
<body>
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			//显示原本的图片
			if(${newhouse.image != null && newhouse.image != ''}) {
				$("#imageShow").attr("height", "400px");
				$("#imageShow").attr("src", "${newhouse.image}");
				$("#imagePage").attr("href", "${newhouse.image}");
			}
	...
	<form id="frm_info" method="post" enctype="multipart/form-data"
		...
		<input type="hidden" name="houseId" value="${newhouse.houseId}" />
		<input type="hidden" id="image" name="image" value="${newhouse.image}" />
		<div style="margin:8px;">
			<span style="width:100px; display:inline-block;">标题：</span> <input
				type="text" id="title" name="title" value="${newhouse.title}"
		...
		<div style="margin:8px;">
			<div style="width:100px;margin:10px 0;">介绍：</div>
			<textarea id="introduce" name="introduce" class="easyui-textbox"
				style="width:800px;height:320px;padding:8px;"
				data-options="multiline:true,prompt:'请输入消息详情...'">${newhouse.introduce}</textarea>
		</div>
		<div style="margin:8px;">
			<div style="width:100px;margin:10px 0;">内容：</div>
			<textarea id="detail" name="detail" class="easyui-textbox"
				style="width:800px;height:320px;padding:8px;"
				data-options="multiline:true,prompt:'请输入消息详情...'">${newhouse.detail}</textarea>
		</div>
	</form>
	<div style="margin:8px 8px 16px 8px;">
		<a href="javascript:saveNewHouseInfo();" class="easyui-linkbutton"
